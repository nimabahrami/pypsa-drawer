...
export class Canvas {
    ...
    _bindEvents() {
        ...
        window.addEventListener('pointerup', () => {
            this.isPanning = false;
            this.dragging = null;
            this.svg.style.cursor = this.placingType ? 'crosshair' : 'default';
        });
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'Delete' || e.key === 'Backspace') {
                if (this.selectedId && document.activeElement.tagName !== 'INPUT' && document.activeElement.tagName !== 'SELECT') {
    ...
    _drawBranchConnection(comp, typeDef) {
        ...
        // Direction arrow for Link on the immediate bus1-connecting segment
        if (comp.type === 'Link' && bus1) {
            const isBidirectional = comp.data.p_min_pu < 0;
            if (!isBidirectional) {
                const bus1Bounds = this._getBounds(bus1);
                const ddx = bus1.cx - comp.cx;
                const ddy = bus1.cy - comp.cy;
                const isVert1 = bus1.rotation === 90 || bus1.rotation === 270;

                // Compute bus1 edge & elbow (same logic as _drawOrthogonalPath)
                let bx1, by1, elbowX, elbowY;
                if (isVert1) {
                    bx1 = bus1.cx + (ddx <= 0 ? bus1Bounds.hw : -bus1Bounds.hw);
                    by1 = Math.max(bus1.cy - bus1Bounds.hh, Math.min(bus1.cy + bus1Bounds.hh, comp.cy));
                    // Elbow is at (tx, by1) where tx = comp edge x
                    elbowX = bx1;
                    elbowY = by1;
                } else {
                    bx1 = Math.max(bus1.cx - bus1Bounds.hw, Math.min(bus1.cx + bus1Bounds.hw, comp.cx));
                    by1 = bus1.cy + (ddy <= 0 ? bus1Bounds.hh : -bus1Bounds.hh);
                    elbowX = bx1;
                    elbowY = by1;
                }

                // Arrow on the last segment: elbow -> bus1 edge
                // For horizontal bus: last segment is horizontal (elbowY==by1, from elbowX to bx1)
                // For vertical bus: last segment is vertical (elbowX==bx1, from elbowY to by1)
                let ax, ay, angle;
                if (isVert1) {
                    // Last segment is vertical: (bx1, elbowY) -> (bx1, by1)
                    // Since elbowX==bx1 and elbowY==by1, use the second segment instead
                    // The L-path is: M(tx,ty) L(tx,by1) L(bx1,by1)
                    // Last segment: (tx, by1) -> (bx1, by1) = horizontal
                    const compBounds = this._getBounds(comp);
                    const tx = comp.cx + (ddx >= 0 ? -compBounds.hw : compBounds.hw);
                    ax = (tx + bx1) / 2;
                    ay = by1;
                    angle = (bx1 - tx) >= 0 ? 0 : 180;
                } else {
                    // The L-path is: M(bx1,by1) L(bx1,ty) L(tx,ty)
                    // First segment from bus1 is vertical: (bx1,by1) -> (bx1,ty)
                    const compBounds = this._getBounds(comp);
                    const ty = comp.cy + (ddy >= 0 ? -compBounds.hh : compBounds.hh);
                    ax = bx1;
                    ay = (by1 + ty) / 2;
                    angle = (ty - by1) >= 0 ? 90 : -90;
                }
